...
Output header will contain protein accession/array no, original accession number, positions and fullname (if included in original fasta), observed stand and the information about flank in terms of which side of the gene is fetched.
      	...
      	'''))
parser.add_argument('-i', '--input', required=True, type=str, dest='filename', help='Specify a fastafile to fetch regions from.\n')
            ...
            if "Accession" not in line:
                ...
                if strand == "1":
                    for record in SeqIO.parse(filename, "fasta"):
                        fullname = record.description.replace(record.id + ' ','')
                        if record.id == acc:
                            if circ == 'Y':
                                if int(start) - flank > 0:
                                    print '>' + prot_acc + ' | '+record.id + ' | ' + str(int(start)-flank) + '-' + str(int(start)-1) + ' | ' + prot_acc + ' | '+ fullname + ' | ' + strand + ' | Left_flank'
                                    seq = Seq(str(record.seq[int(start)-(flank+1):int(start)-1]))
                                    print re.sub("(.{60})", "\\1\n", str(seq), 0, re.DOTALL)
                                else:
                                    print '>' + prot_acc + ' | '+record.id + ' | ' + str(len(record.seq) - (flank - int(start))) + '-' + str(len(record.seq)) + ' ~ 1-' + str(
                                        int(start) - 1) + ' | ' + prot_acc + ' | ' + fullname + ' | ' + strand + ' | Left_flank'
                                    seq = Seq(str(record.seq[0:int(start) - 1]))
                                    seq_round = Seq(str(record.seq[len(record.seq) - ((flank+1) - int(start)):len(record.seq)]))
                                    seq_cat = seq_round + seq
                                    print re.sub("(.{60})", "\\1\n", str(seq_cat), 0, re.DOTALL)
                                if promoter == 'N':
                                    if int(stop) + flank < len(record.seq):
                                        print '>' + prot_acc + ' | '+record.id + ' | ' + str(int(stop)+1) + '-' + str(
                                            int(stop) + flank) + ' | ' + prot_acc + ' | ' + fullname + ' | ' + strand + ' | Right_flank'
                                        seq = Seq(str(record.seq[int(stop) :int(stop) + flank]))
                                        print re.sub("(.{60})", "\\1\n", str(seq), 0, re.DOTALL)
                                    else:
                                        print '>' + prot_acc + ' | '+record.id + ' | ' + str(int(stop)+1) + '-' + str(
                                            len(record.seq)) + ' ~ 1-' + str(
                                            flank - (len(record.seq) - int(stop))) + ' | ' + prot_acc + ' | ' + fullname + ' | ' + strand + ' | Right_flank'
                                        seq = Seq(str(record.seq[0:(flank - (len(record.seq) - int(stop)))]))
                                        seq_round = Seq(str(record.seq[int(stop):]))
                                        seq_cat = seq_round + seq
                                        print re.sub("(.{60})", "\\1\n", str(seq_cat), 0, re.DOTALL)
                                ...
                                if int(start) - flank > 0:
                                    ...
                                    print '>' + prot_acc + ' | '+record.id + ' | ' + '1-' + str(
                                        int(start) - 1) + ' | ' + prot_acc + ' | ' + fullname + ' | ' + strand + ' | Left_flank'
                                    seq = Seq(str(record.seq[0:int(start) - 1]))
                                    print re.sub("(.{60})", "\\1\n", str(seq), 0, re.DOTALL)
                                ...
                                if promoter == 'N':
                                    if int(stop) + flank < len(record.seq):
                                        ...
                                        print '>' + prot_acc + ' | '+record.id + ' | ' + str(int(stop)+1) + '-' + str(
                                            len(record.seq)) + ' | ' + prot_acc + ' | ' + fullname + ' | ' + strand + ' | Right_flank'
                                        seq_round = Seq(str(record.seq[int(stop):]))
                        ...
                        if record.id == acc:
                            if circ == 'Y':
                                if int(stop) + flank < len(record.seq):
                                    ...
                                    seq = Seq(str(record.seq[int(stop) :int(stop) + flank])).reverse_complement()
                                    print re.sub("(.{60})", "\\1\n", str(seq), 0, re.DOTALL)
                                else:
                                    print '>' + prot_acc + ' | '+record.id + ' | ' + str(int(stop) + 1) + '-' + str(len(record.seq)) + ' ~ 1-' + str(int(stop) + flank - len(record.seq)) + ' | ' + prot_acc + ' | ' + fullname + ' | ' + strand + ' | Left_flank'
                                    seq = Seq(str(record.seq[int(stop):int(stop) + flank])).reverse_complement()
                                    seq_round = Seq(str(record.seq[0:int(stop) + flank - len(record.seq)])).reverse_complement()
                                    seq_cat = seq_round + seq
                                    print re.sub("(.{60})", "\\1\n", str(seq_cat), 0, re.DOTALL)

                                if promoter == 'N':
                                    if int(start) - flank > 0:
                                        print '>' + prot_acc + ' | '+record.id + ' | ' + str(int(start)-flank) + '-' + str(
                                            int(start) -1) + ' | ' + prot_acc + ' | ' + fullname + ' | ' + strand + ' | Right_flank'
                                        seq = Seq(str(record.seq[int(start) - flank -1 :int(start) -1])).reverse_complement()
                                        print re.sub("(.{60})", "\\1\n", str(seq), 0, re.DOTALL)
                                    else:
                                        print '>' + prot_acc + ' | '+record.id + ' | ' + str(len(record.seq) - (flank - int(start))) + '-' + str(
                                            len(record.seq)) + ' ~ 1-' + str(int(start) -1) + ' | ' + prot_acc + ' | ' + fullname + ' | ' + strand + ' | Right_flank'
                                        seq = Seq(str(record.seq[len(record.seq) - (flank - int(start)) - 1:])).reverse_complement()
                                        seq_round = Seq(
                                            str(record.seq[0:int(start)-1])).reverse_complement()
                                        seq_cat = seq_round + seq
                                        print re.sub("(.{60})", "\\1\n", str(seq_cat), 0, re.DOTALL)
                                ...
                                if int(stop) + flank < len(record.seq):
                                    ...
                                    print '>' + record.id + ' | ' + str(int(stop) + 1) + '-' + str(
                                        len(record.seq)) + ' | ' + prot_acc + ' | ' + fullname + ' | ' + strand + ' | Left_flank'
                                    seq = Seq(str(record.seq[int(stop):int(stop) + flank])).reverse_complement()
                                    print re.sub("(.{60})", "\\1\n", str(seq), 0, re.DOTALL)
                                ...
                                if promoter == 'N':
                                    if int(start) - flank > 0:
                                        ...
                                        print '>' + prot_acc + ' | '+record.id + ' | 1-' + str(int(start)-1) + ' | ' + prot_acc + ' | ' + fullname + ' | ' + strand + ' | Right_flank'
                                        seq = Seq(str(record.seq[0:(int(start)-1)])).reverse_complement()
                                        print re.sub("(.{60})", "\\1\n", str(seq), 0, re.DOTALL)
